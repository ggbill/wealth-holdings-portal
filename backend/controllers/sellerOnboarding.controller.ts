...
export namespace SellerOnboardingController {
    ...
    export async function GetLatestDataForActiveCases(): Promise<any> {
        return new Promise((resolve: (result: any) => void, reject: (error: Error) => void) => {
            SellerOnboardingWebhook.aggregate([
                { $sort: { _modified_at: 1 } },
                {
                    $group: {
                        _id: {
...
export async function GetInstanceDetails(kissflowId: string): Promise<any> {
    ...
    return new Promise((resolve: (result: any) => void, reject: (error: Error) => void) => {
        ...
        SellerOnboardingWebhook.find({ _kissflow_id: kissflowId }, function (err, result) {
            if (err) {
                console.error("Error: " + err);
            }
            resolve(result);
...
export async function GetActions(): Promise<any> {
    return new Promise((resolve: (result: any) => void, reject: (error: Error) => void) => {
        ...
        SellerOnboardingWebhook.find({}, function (err, result) {
            if (err) {
                console.error("Error: " + err);
            }
            resolve(result);
